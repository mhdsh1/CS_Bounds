...
global alvaro = "${path}/input/alvaro"
...
* Create temp files for cpi deflators: this is taken from clean_adiminstrativedata_QJE.do

use "data/data/cpiursai1977-2016.dta", clear
...
tempfile cpiquarter
save `cpiquarter'
use "data/data/qcew_multiplier.dta", clear
...
tempfile CPS_cleaned_10_15
save `CPS_cleaned_10_15'
...
//keep if year == 2007 | year == 2010 | year == 2015
keep if inrange(year, 2010, 2015)
tempfile CPS_cleaned_deflated_10_15
save `CPS_cleaned_deflated_10_15'


*** Now, we create the dummy variables for wage increases in 2007-10 and 2010-15. We use VZ_mw_state_quarterly_new.dta which contains state-level minimum wages at  quarterly frequency

use "data/data/VZ_mw_state_quarterly_new.dta", clear 
...
save "data/yearly_minwage_increase.dta", replace //Save yearly minimum wage increase

*** We have saved yearly minimum wage increase, now do the same with 2007-10 and 2010-15
...
save "data/minwage_increase_10_15_mahdi.dta", replace //Save ever minimum wage increase

use "data/data/state_level_quarterly_unemp_rate.dta" 
...
/* Mahdi: i don't find the presidential data 

*** Create the data for the last presidential election (i.e 2007  refers to 2004 election, 2010 to 2008 and so on). The data on this is contained  in presidential_elections_from 1976_2016.dta. I also make a yearly presidential election file for the yearly cleaned and merged data.

use "data/data/presidential_elections_from 1976_2016.dta", clear
preserve
sort statenum year
...
use `CPS_cleaned_10_15', clear //Back to the CPS data to merge the binary variables

merge m:1 statenum using "data/minwage_increase_10_15.dta", nogenerate 
//Merge minimum wage increase

merge m:1 statenum using "data/data/state_ideologyshares.dta", nogenerate 
//Merge state ideology share

merge m:1 statenum year using "data/data/state_demog.dta", nogenerate 
...
merge m:1 statenum year using "data/yearly_minwage_increase.dta", ///
nogenerate keepusing(state_mw)

keep if year == 2007 | year == 2010 | year == 2015
save "data/CPS_cleaned_merged_07_15.dta", replace
export delimited "data/CPS_cleaned_merged_07_15.csv", replace
...
use `CPS_cleaned_deflated_07_15', clear //Back to the CPS data to merge the binary variables
...
keep if year == 2007 | year == 2010 | year == 2015
save "data/CPS_cleaned_merged_deflated_07_15.dta", replace
export delimited "data/CPS_cleaned_merged_deflated_07_15.csv", replace


use `CPS_cleaned_yearly', clear 
//Do the same to merge with the yearly CPS data/CPS_cleaned_07_15

merge m:1 statenum year using "data/yearly_minwage_increase.dta", nogenerate 
...
save "data/CPS_cleaned_merged_yearly.dta", replace

export delimited "data/CPS_cleaned_merged_yearly.csv", replace

preserve

describe, replace

export excel using "variable_descriptions_cps_cleaned_merged_yearly.xlsx", replace first(var)
...
save "data/CPS_cleaned_deflated_merged_yearly.dta", replace

export delimited "data/CPS_cleaned_deflated_merged_yearly.csv", replace
