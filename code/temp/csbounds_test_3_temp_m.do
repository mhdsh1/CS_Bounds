...
**** making the first row of Table 2 for subgroup 2
...
mata:
    ...
    results = (fy1, cslb, csub, did, distdid)	
    st_matrix("results", results)

    fy1     = round(EQY11Tu, 0.01)
    cslb    = round(EQY11Tu - EQY10TCSLBu, 0.01)
    csub    = round(EQY11Tu - EQY10TCSUBu, 0.01)
    ...
    resultsu = (fy1, cslb, csub, distdid)
    st_matrix("resultsu", resultsu)

    end
/*
matrix colnames results = fy1 cslb csub distdid
matrix rownames results = "Mean"
matrix list results
	
matrix colnames resultsu = fy1 cslb csub distdid
matrix rownames resultsu = "LT Mean for u= `u'"
