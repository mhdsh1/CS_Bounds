...
print(round(EY11T - EY10TDID, 2))

EY10TCSLB = sum(
  QY10TCSLB[QY10TCSLB<=maxR & QY10TCSLB>=min(Ysupp1[-c(1)])]
  )*pgrid # EY10TCSLB -- this should be UB maybe ... typo somewhere? 
print(round(EY11T - EY10TCSLB, 2))

EY10TCSUB = sum(
  QY10TCSUB[QY10TCSUB<=maxR & QY10TCSUB>= min(Ysupp1[-c(1)])]
  )*pgrid
print(round(EY11T - EY10TCSUB, 2))
...
A =sum(QY11T[p<=1&p>0])*pgrid/1
A =sum(QY11T[p<=0.99&p>0])*pgrid/1
